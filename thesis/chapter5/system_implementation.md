...
本章将基于第4章的系统架构概要设计，详细介绍 GalRemote 系统各核心模块的具体工程落地过程与关键技术实现。在跨系统平台与多语言混合编程的复杂环境下，本系统通过 C++ 底层的高效资源调度、Rust 的并发中台管理以及 Vue 3 现代化响应式前端，建立了一套低延迟、高可靠的流媒体远端游玩与存档管理方案。本章将对上述三层架构中的核心算法流与关键代码片段进行剖析。
...
由于各种不同形态的移动端设备的屏幕纵横比与物理分辨率大相径庭，单纯的绝对坐标透传不可避免地会导致光标漂移。因此，系统在底层抽象了 `touch_port_t` 触控视口结构体。
